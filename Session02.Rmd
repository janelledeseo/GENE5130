...
sample_info <- metadata %>%
  select(-geo_id) %>%
  dplyr::rename(dexamethasone = dex) # rename(new name = old name)
sample_info
...
#dds
# You can recover the original counts matrix with the counts function! The code below prints the columns
counts(dds)
```
...
**Write your thoughts here: Outliers may skew the mean, experimental errors are not significant, remove random noise**
...
It will normally take the first one alphabetical, but to make sure you can use the relevel function! Look at this example, edit according to what you are working with
```{r}
...
run counts() again on the object created from the main DESEq function, create two code chunks, run the counts() method twice but add normalized=T to one of them, look at the differences, which ones are scaled up? which ones are scaled down?
```{r}
...
How many genes are up-regulated?? How many are down-regulated??

Use the order() function to sort the res object by adjusted p values
